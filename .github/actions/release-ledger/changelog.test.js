...
test('verified deployment can be a baseline while feature wording is pending',()=>{const m={target:'web',commit:'a'.repeat(40)},row={properties:{Manifest:rich(JSON.stringify(m)),'Manifest Hash':rich(hash(m)),State:{select:{name:'Waiting'}},Observation:rich(JSON.stringify({phase:'deployed',verification:{kind:'http',commit:m.commit,evidence:'https://production.test'}}))}};assert.equal(deployedBaseline(row),true);row.properties.Observation=rich(JSON.stringify({phase:'uploaded'}));assert.equal(deployedBaseline(row),false)});
