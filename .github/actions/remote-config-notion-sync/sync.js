...
function validateAndroidFirebaseAccount(config, account, monitorEmail, enabled = false) {
  if(!enabled||config.product!=='sap'||config.platform!=='android')return;
...
async function main() {
  ...
  validateAndroidFirebaseAccount(config, serviceAccount, process.env.PLAY_MONITOR_SERVICE_ACCOUNT_EMAIL, process.env.INPUT_VALIDATE_ANDROID_CREDENTIALS === 'true');
  const clients = {
