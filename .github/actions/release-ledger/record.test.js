...
test('atomic shared ref excludes concurrent publishers and releases owner lock',async()=>{let locked=false,active=0,max=0;const gh=async(p,m,b)=>{if(p.endsWith('/heads/main'))return {object:{sha:'a'.repeat(40)}};if(p.endsWith('/git/tags'))return {sha:b.message};if(p.endsWith('/git/refs')&&m==='POST'){if(locked)throw Object.assign(new Error('exists'), {status:422});locked=b.sha;return {}};if(m==='DELETE'){locked=false;return{}};return {object:{sha:locked}}};const fn=()=>withLock(gh,'repo',async()=>{active++;max=Math.max(max,active);await new Promise(r=>setTimeout(r,20));active--},{delay:5});const results=await Promise.allSettled([fn(),fn()]);assert.equal(results.filter(x=>x.status==='fulfilled').length,2);assert.equal(max,1);assert.equal(locked,false)});
