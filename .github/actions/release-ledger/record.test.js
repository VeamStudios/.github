...
test('malformed shipped PR links are recorded as errors while valid siblings remain in the manifest', async () => {
  ...
  try {
    ...
    const config = { repo, target: 'web', product: 'Test', version: 'v1', commit: shipped, baseline, event: 'release', source: good.html_url, phase: 'prepare', releasesId: 'releases', dryRun: true };
    const api = { gh, notion: async () => ({ results: [], has_more: false }) };
